...
describe('UserValidator unit tests', () => {
  ...
  it('Invalidation cases for name field', () => {
    ...
    isValid = sut.validate({ ...props, name: 10 as unknown as string });
    expect(isValid).toBeFalsy();
    expect(sut.errors['name']).toStrictEqual([
      'name must be a string',
      'name must be shorter than or equal to 255 characters',
  ...
  it('Invalidation cases for email field', () => {
    ...
    isValid = sut.validate({ ...props, email: 10 as unknown as string });
    expect(isValid).toBeFalsy();
    expect(sut.errors['email']).toStrictEqual([
      'email must be an email',
      'email must be a string',
  ...
  it('Invalidation cases for password field', () => {
    ...
    isValid = sut.validate({ ...props, password: 10 as unknown as string });
    expect(isValid).toBeFalsy();
    expect(sut.errors['password']).toStrictEqual([
      'password must be a string',
      'password must be shorter than or equal to 100 characters',
  ...
  it('Invalidation cases for createdAt field', () => {
    ...
    isValid = sut.validate({ ...props, createdAt: '2023' as unknown as Date });
    expect(isValid).toBeFalsy();
    expect(sut.errors['createdAt']).toStrictEqual([
      'createdAt must be a Date instance',
    ]);
